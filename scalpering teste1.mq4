...
extern double Hiden_TP = 50;
extern double Lot = 0.01;
extern double TakeProfit = 50; // TakeProfit em pontos
extern double StopLoss = 50; //StopLoss   em pontos
...
int start() {
   ...
   */
   ...
   L_i_5 = (int)MarketInfo(_Symbol, MODE_DIGITS);
     ...
     // Calcular a média móvel simples de 50 períodos
   sma = iMA(Symbol(), 0, smaPeriod, 0, MODE_SMA, PRICE_CLOSE, 0);
   
   // Determinar a direção da tendência
   bool uptrend = Close[0] > sma;
   bool downtrend = Close[0] < sma;
            ...
            do {
               ...
               if (OrderSymbol() == _Symbol) {
                  if (OrderSymbol() == _Symbol && OrderMagicNumber() == I_i_71) {
                      if (OrderType() == OP_BUY) {
                        I_b_7 = OrderClose(OrderTicket(), OrderLots(),  Ask - StopLoss, (int)I_d_34, 16711680);
                     }
                     if (OrderType() == OP_SELL) {
                        I_b_7 = OrderClose(OrderTicket(), OrderLots(),  Bid + StopLoss, (int)I_d_34, 255);
                     }
                  }
                  Sleep(1000);
               }
